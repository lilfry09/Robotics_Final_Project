...
def hdf5_vector(obs: h5py.Group, key: str, t: int, expected_dim: int) -> np.ndarray:
    if key not in obs:
        raise KeyError(f"Target {key!r} requires obs/{key} in the HDF5 file")
    value = np.asarray(obs[key][t], dtype=np.float32).reshape(-1)
    if value.shape[0] != expected_dim:
        raise ValueError(f"Expected obs/{key} dim {expected_dim}, got {value.shape[0]}")
    if not np.isfinite(value).all():
        raise ValueError(f"Non-finite values found in obs/{key} at timestep {t}")
    return value
        ...
        with h5py.File(hdf5_path, "r") as f:
            for demo_key in iter_demo_keys(f):
                ...
                for t in range(0, usable_length, stride):
                    ...
                    elif target_name in ("contact_xyz", "contact_distance"):
                    ...
                    elif target_name == "ee_to_object_xyz":
                        target = hdf5_vector(obs, "ee_to_object_xyz", t, 3)
                    elif target_name == "object_to_target_xyz":
                        target = hdf5_vector(obs, "object_to_target_xyz", t, 3)
                    elif target_name == "gripper_to_contact_distance":
                        target = hdf5_vector(obs, "gripper_to_contact_distance", t, 1)
                    elif target_name == "task_3d":
                        target = np.concatenate(
                            [
                                hdf5_vector(obs, "ee_to_object_xyz", t, 3),
                                hdf5_vector(obs, "object_to_target_xyz", t, 3),
                                hdf5_vector(obs, "gripper_to_contact_distance", t, 1),
                            ]
                        ).astype(np.float32)
                    else:
                        raise ValueError(f"Unhandled target {target_name!r}")
