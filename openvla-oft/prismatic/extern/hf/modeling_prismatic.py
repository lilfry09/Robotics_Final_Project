...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    ...
    def _process_depth_features(
        ...
        """Process depth maps into geometry tokens and append to vision features."""
    ...
    def _encode_depth_context(
        ...
        if summary:
            ...
            if hasattr(depth_encoder, "compute_point_features"):
    ...
    def _encode_depth_point_features(
        self,
        projected_patch_embeddings,
        depth_values=None,
        depth_intrinsics=None,
        ...
        if depth_encoder is None or not hasattr(depth_encoder, "compute_point_features"):
            return None
        point_features = depth_encoder.compute_point_features(
            depth_values=depth_values,
            depth_intrinsics=depth_intrinsics,
            depth_extrinsics=depth_extrinsics,
            depth_valid_mask=depth_valid_mask,
        ...
        return point_features.to(device=projected_patch_embeddings.device, dtype=torch.float32)

    def get_num_prefix_tokens(
        self,
        use_depth: bool = False,
        depth_num_tokens: int = 0,
        use_proprio: bool = False,
        use_diffusion: bool = False,
    ) -> int:
        """Return number of prefix tokens prepended before text/action tokens."""
        num_prefix_tokens = self.vision_backbone.get_num_patches() * self.vision_backbone.get_num_images_in_input()
        if use_depth:
            num_prefix_tokens += depth_num_tokens
        if use_proprio:
            num_prefix_tokens += 1
        if use_diffusion:
            num_prefix_tokens += 1
    ...
    def predict_action(
        ...
        **kwargs: str,
        ...
        if use_depth_prefix:
            ...
            depth_point_features = self._encode_depth_point_features(
                projected_patch_embeddings,
                depth_values=depth_values,
                depth_intrinsics=depth_intrinsics,
                depth_extrinsics=depth_extrinsics,
