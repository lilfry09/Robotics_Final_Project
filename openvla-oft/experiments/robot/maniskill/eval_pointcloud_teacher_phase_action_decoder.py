#!/usr/bin/env python3
"""Closed-loop eval for the raw-pointcloud teacher phase/action decoder."""
...
def choose_points(
    ...
    if point_mode == "rgb_only":
        points = extract_points(obs, num_points, rng, min_z)
        points[..., :3] = 0.0
        return points
...
def predict_action(
    ...
    action = denormalize_actions(pred_norm.detach().cpu().numpy(), norm)[0].astype(np.float32)
    return action, phase_idx, float(probs[phase_idx])
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for episode_idx in range(args.episodes):
        ...
        for _ in range(args.max_steps):
            ...
            action, phase_idx, phase_conf = predict_action(model, norm, points, task_feature, device)
    ...
    result = {
        ...
        "device": str(device),
        "note": "Raw pointcloud/task teacher-distilled phase/action decoder; not OpenVLA.",
    }
...
def main() -> None:
    ...
    parser.add_argument("--point_mode", choices=["normal", "null", "cross_demo", "rgb_only"], default="normal")
