...
def get_vla(cfg: Any) -> torch.nn.Module:
    ...
    base_checkpoint = getattr(cfg, "base_model_checkpoint", None) or cfg.pretrained_checkpoint
    adapter_dir = os.path.join(checkpoint_path, "lora_adapter")
    load_lora_adapter = os.path.isdir(adapter_dir)
    ...
    if not model_is_on_hf_hub(str(base_checkpoint)):
    ...
    _load_dataset_stats(vla, checkpoint_path)
...
def _load_dataset_stats(vla: torch.nn.Module, checkpoint_path: str) -> None:
    ...
    if os.path.isfile(dataset_statistics_path):
        ...
        if hasattr(vla, "get_base_model"):
            vla.get_base_model().norm_stats = norm_stats
...
def get_processor(cfg: Any) -> AutoProcessor:
    ...
    """
    processor_checkpoint = getattr(cfg, "processor_checkpoint", None) or getattr(cfg, "base_model_checkpoint", None)
    if processor_checkpoint is None:
        processor_checkpoint = cfg.pretrained_checkpoint
    return AutoProcessor.from_pretrained(processor_checkpoint, trust_remote_code=True)
...
def get_depth_encoder(cfg: Any, llm_dim: int) -> LightweightDepthTokenEncoder:
    """Load the DepthVLA geometry encoder from a local checkpoint directory."""
    geometry_norm = getattr(cfg, "geometry_norm", "none")
    geometry_clip = getattr(cfg, "geometry_clip", 5.0)
    depth_encoder_type = getattr(cfg, "depth_encoder_type", "grid")
    if depth_encoder_type == "grid":
        depth_encoder = LightweightDepthTokenEncoder(
            llm_dim=llm_dim,
            hidden_dim=getattr(cfg, "depth_hidden_dim", 256),
            grid_size=getattr(cfg, "depth_grid_size", 4),
            depth_min_m=getattr(cfg, "depth_min_m", 0.01),
            depth_max_m=getattr(cfg, "depth_max_m", 5.0),
            num_views=2,
    ...
    elif depth_encoder_type == "dense_point":
        if geometry_norm != "none":
            raise ValueError("depth_encoder_type='dense_point' requires geometry_norm='none'")
        depth_encoder = DensePointDepthTokenEncoder(
            llm_dim=llm_dim,
            hidden_dim=getattr(cfg, "depth_hidden_dim", 256),
            num_points_per_view=getattr(cfg, "depth_num_points_per_view", 1024),
            depth_min_m=getattr(cfg, "depth_min_m", 0.01),
            depth_max_m=getattr(cfg, "depth_max_m", 5.0),
            num_views=2,
        ).to(DEVICE)
    ...
    if model_is_on_hf_hub(cfg.pretrained_checkpoint):
        raise ValueError("DepthVLA depth_encoder loading is only supported for local checkpoints.")
    checkpoint_path = find_checkpoint_file(cfg.pretrained_checkpoint, "depth_encoder")
    state_dict = load_component_state_dict(checkpoint_path)
    depth_encoder.load_state_dict(state_dict)

    if geometry_norm == "dataset_std":
        stats_path = Path(cfg.pretrained_checkpoint) / "geometry_norm_stats.json"
        if not stats_path.exists():
            raise FileNotFoundError(f"geometry_norm='dataset_std' but stats file is missing: {stats_path}")
        with open(stats_path, "r") as f:
            stats = json.load(f)
        depth_encoder.set_geometry_normalization(stats, geometry_norm, geometry_clip)
        print(f"Loaded DepthVLA geometry normalization stats from: {stats_path}")
        print(f"  mean={stats['mean']}")
        print(f"  std={stats['std']}")
        print(f"  clip={geometry_clip}")
    elif geometry_norm != "none":
...
def get_action_head(cfg: Any, llm_dim: int) -> Union[L1RegressionActionHead, DiffusionActionHead]:
    ...
    if cfg.use_l1_regression:
        action_head = L1RegressionActionHead(
            ...
            depth_hidden_delta_clip=getattr(cfg, "depth_hidden_delta_clip", 0.0),
            depth_action_residual_clip=getattr(cfg, "depth_action_residual_clip", 0.0),
            depth_keypose_residual_weight=getattr(cfg, "depth_keypose_residual_weight", 0.0),
            depth_keypose_residual_clip=getattr(cfg, "depth_keypose_residual_clip", 0.0),
            depth_point_action_weight=getattr(cfg, "depth_point_action_weight", 0.0),
            depth_point_action_clip=getattr(cfg, "depth_point_action_clip", 0.0),
            depth_waypoint_action_weight=getattr(cfg, "depth_waypoint_action_weight", 0.0),
            depth_waypoint_action_clip=getattr(cfg, "depth_waypoint_action_clip", 0.0),
            depth_waypoint_action_scale=getattr(cfg, "depth_waypoint_action_scale", 1.0),
            depth_waypoint_action_chunk_len=getattr(cfg, "depth_waypoint_action_chunk_len", 1),
...
def get_vla_action(
    ...
    depth_encoder: Optional[torch.nn.Module] = None,
    use_film: bool = False,
) -> List[np.ndarray]:
    """
    ...
    with torch.inference_mode():
        ...
        if getattr(cfg, "use_depth", False):
            depth_kwargs = {
                "depth_values": torch.tensor(obs["depth_values"], dtype=torch.float32, device=DEVICE).unsqueeze(0),
                "depth_intrinsics": torch.tensor(obs["depth_intrinsics"], dtype=torch.float32, device=DEVICE).unsqueeze(0),
        ...
        if action_head is None:
            ...
            action, _ = vla.predict_action(
                **inputs, **depth_kwargs, unnorm_key=cfg.unnorm_key, do_sample=False, depth_encoder=depth_encoder
            )
        else:
